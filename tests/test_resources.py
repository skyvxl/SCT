...
from scmm.logging_config import configure_logging, log_file_path
from scmm.resource_loader import (
    load_optional_icon,
    load_stylesheet,
...
class ResourceTests(unittest.TestCase):
    ...
    def test_optional_resource_returns_none_when_absent(self) -> None:
        with self.assertLogs("scmm.resources", level="WARNING") as logs:
    ...
    def test_corrupt_icon_falls_back_to_none_with_warning(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("scmm.resource_loader.optional_resource_path", return_value=corrupt_icon):
                with self.assertLogs("scmm.resources", level="WARNING"):
    ...
    def test_log_path_uses_local_app_data(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with patch.dict(os.environ, {"LOCALAPPDATA": directory}):
                expected = Path(directory) / "SeamlessCoopModManager" / "logs" / "scmm.log"
                self.assertEqual(log_file_path(), expected)

    def test_missing_stylesheet_falls_back_to_empty_qss(self) -> None:
        missing = Path(tempfile.gettempdir()) / "missing-scmm-theme.qss"
        with patch("scmm.resource_loader.resource_path", return_value=missing):
            with self.assertLogs("scmm.resources", level="WARNING"):
                self.assertEqual(load_stylesheet(), "")

    def test_logging_uses_stderr_when_file_handler_cannot_open(self) -> None:
        logger = logging.getLogger("scmm")
        original_handlers = logger.handlers[:]
        original_level = logger.level
        original_propagate = logger.propagate
        try:
            with patch("scmm.logging_config.logging.FileHandler", side_effect=OSError):
